...
class AutoBackupTest(_PluginBase):
    """
    自动备份测试插件
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "autobackuptest_"
    # 加载顺序
    plugin_order = 17
    # 可使用的用户级别
    auth_level = 1
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._onlyonce:
            ...
            logger.info(f"自动备份测试服务启动，立即运行一次")
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [{
            ...
            "methods": ["GET"],
            "summary": "MoviePilot备份测试",
            "description": "MoviePilot备份测试",
        }]
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'info',
                                            'variant': 'tonal',
                                            'text': '备份文件路径默认为本地映射的config/plugins/AutoBackupTest。'
                                        }
                                    }
                                ]
                            }
